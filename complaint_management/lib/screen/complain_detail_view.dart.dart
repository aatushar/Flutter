...
import '../model/complain_image_list_model.dart';
...
class ComplainDetailViewPage extends StatefulWidget {
  ComplainDetailViewPage({Key? key, required this.ComplainID}) : super(key: key);
  final ComplainID;
  @override
  State<ComplainDetailViewPage> createState() => _ComplainDetailViewPageState(ComplainID:ComplainID);
}

class _ComplainDetailViewPageState extends State<ComplainDetailViewPage> {
  _ComplainDetailViewPageState({Key? key, required this.ComplainID});
  final ComplainID;
  int _page = 0;
  ...
  List<ComplainImageListModel> ComplainImageList = [];
  @override
  void initState() {
                                            ...
                                            children: [
                                              ...
                                              Expanded(child: Column(
                                                ...
                                                children: [
                                                  ...
                                                  Text(ComplainList[index].title.toString(),style:TextStyle(fontSize: 22,fontWeight: FontWeight.bold)),
                                                  SizedBox(height: 2),
                                                  Divider(height: 1,color:Colors.black38),

                                                  SizedBox(height: 5),
                                                  Text(ComplainList[index].description.toString(),style:TextStyle(fontSize: 15))
                                                ],
                                              )),
                  ...
                  builder: (context,snapshort){
                      if(!snapshort.hasData){
                        ...
                        return GridView.count(
                          ...
                          children: List.generate(ComplainImageList.length, (index) {
                            return Padding(
                              padding: const EdgeInsets.all(3.0),
                              child: Container(
                                height:60,
                                width: 60,
                                child: Image.network('https://scb-bd.com/apis_app/public/images/complain/'+ComplainImageList[index].images.toString(),fit: BoxFit.cover),
                              ),
                            );
                          }),
                        );
  ...
  Future<List<ComplainListModel>> GetNotificationData() async {
    ...
    var response = await http.get(Uri.parse("http://165.232.154.198/ccb/api/get-complain-list?id="+ComplainID.toString()));
  ...
  Future<List<ComplainImageListModel>> GetComplainImageData() async {
    EasyLoading.show(status: 'loading...');
    var response = await http.get(Uri.parse("http://165.232.154.198/ccb/api/get-complain-list?type=image&id="+ComplainID.toString()));
    var data = jsonDecode(response.body.toString());
    ComplainImageList = [];
    if(response.statusCode == 200){
      for(Map i in data){
        ComplainImageList.add(ComplainImageListModel.fromJson(i));
      }
      return ComplainImageList;
    }else{
      return ComplainImageList;
    }
    EasyLoading.dismiss();
