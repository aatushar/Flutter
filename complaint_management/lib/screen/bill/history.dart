...
class _PaymentHistoryState extends State<PaymentHistory> {
    ...
    return Scaffold(
      ...
      body: Container(
        ...
        child:Column(
          ...
          children: <Widget>[
            SizedBox(height: 10),
            Text('last 50 bill history',style:TextStyle(fontSize: 16.0,color: Palette.textClrGray,fontWeight: FontWeight.bold)),
                        ...
                        itemBuilder: (context, index){
                          return Card(
                              child:ExpansionTile(
                                ...
                                // subtitle: Text(PaymentHistoryList[index].createdAt.toString()),
                                trailing: Text(PaymentHistoryList[index].currency.toString()+' '+PaymentHistoryList[index].totalAmount.toString()+'/-',style: TextStyle(fontWeight: FontWeight.bold),),
                                children: <Widget>[
                                  Container(
                                      ...
                                      child: Column(
                                        children: [
                                          ...
                                          //       children: [
                                          //         Text('Invoice: '),
                                          //         Text(PaymentHistoryList[index].invoiceNo.toString()),
                                          //       ],
                                          //     ),
                                          //     Row(
                                          //       children: [
  ...
  Future<List<UserPaymentHistoryModel>> GetPaymentHistoryData() async {
    ...
    PaymentHistoryList = [];
    if(response.statusCode == 200){
      print("http://165.232.154.198/ccb/api/payment-history-v2?LoginID=$LoginID&revSource=$_revSource&userID=$userID");
      for(Map i in data){
        PaymentHistoryList.add(UserPaymentHistoryModel.fromJson(i));
      }
      print(PaymentHistoryList.length);
  ...
  // Future<void> _getInvoicePDF(invoiceID) async {
  //   print('inv '+invoiceID);
  //   final Uri invoicePdfUrl = Uri.parse('https://scb-bd.com/apis_app/api/sr/user-payment-pdf/'+invoiceID);
  //   if(!await launchUrl(invoicePdfUrl)) {
